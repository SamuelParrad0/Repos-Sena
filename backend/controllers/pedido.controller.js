 ...
 * devuelve el stock am los productos
 */
...
const cancelarPedido = async (req, res) => {
    ...
    try {
        ...
        //buscar pedido solo por propios pedidos
        const pedido = await Pedido.findOne({
            where: {
                id,
                usuarioId: req.usuario.id
        ...
        //solo puede cancelar si esta en pendiente
        if (pedido.estado !== 'pendiente') {
            await t.rollback();
            return res.status(400).json({
                success: false,
                message: `solo se puede cancelar si esta en estado ${pedido.estado}`
            });
        }
